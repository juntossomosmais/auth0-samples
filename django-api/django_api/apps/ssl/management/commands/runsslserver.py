...
if (
    ...
    or (sys.version_info[0] == 3 and sys.version_info[1] == 5 and sys.version_info[2] >= 3)
    or (sys.version_info[0] == 2 and sys.version_info[1] == 7 and sys.version_info[2] >= 13)
):
...
def default_ssl_files_dir():
    import django_api.apps.ssl as app_module
...
class Command(runserver.Command):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--key", default=os.path.join(default_ssl_files_dir(), "development.key"), help="Path to the key file"
        ),
        parser.add_argument(
            "--nostatic",
        ...
        parser.add_argument(
            "--static", dest="use_static_handler", action="store_true", help="Use internal static file handler"
        ),
    ...
    def inner_run(self, *args, **options):
        ...
        try:
            ...
            server = SecureHTTPServer(
                (self.addr, int(self.port)), WSGIRequestHandler, cert_file, key_file, ipv6=self.use_ipv6
            )
